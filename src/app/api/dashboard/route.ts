...
export async function GET(request: NextRequest) {
  try {
    ...
    if (role === 'EMPLOYEE') {
      ...
      type G = (typeof goals)[number]
      type CI = G['checkIns'][number]
      totalGoals = goals.length
      approvedGoals = goals.filter((g: G) => g.status === 'APPROVED').length
      pendingApprovals = goals.filter((g: G) => g.status === 'SUBMITTED').length
      const allCheckIns = goals.flatMap((g: G) => g.checkIns)
      checkInsCompleted = allCheckIns.length
      const scores = allCheckIns.filter((c: CI) => c.score != null).map((c: CI) => c.score!)
      ...
      const teamIds = teamMembers.map((u: { id: string }) => u.id)
      const goals = await prisma.goal.findMany({
